--- public/js/Community.js (原始)


+++ public/js/Community.js (修改后)
/* ============================================================
   FPP v2 — Community.js
...
(function () {
  ...
  var likedCache = {}; /* key: type_docId -> bool */
  ...
  function cardHTML(o) {
    var img = o.image ? '<img src="' + UI.esc(o.image) + '" alt="" loading="lazy" onerror="this.parentElement.innerHTML=\'\'">' : '';
  ...
  function renderComHome() {
    ...
    var evBox = document.getElementById('comEventList');
    if (!S.patches.length) UI.empty(document.getElementById('comPatchList'), { title: '등록된 패치노트가 없습니다.' });
    else {
      var el = document.getElementById('comPatchList');
      el.innerHTML = '<ul class="lst">' + S.patches.slice(0, 5).map(function (p) {
    ...
    var bl = document.getElementById('comBoardList');
    ...
    UI.watchReveals(document.getElementById('view-comhome'));
  }
  ...
  function renderPatchList() {
    var el = document.getElementById('patchContent');
  ...
  function renderPatchDetail(id) {
    ...
    likeState('patch', p.docId, p.raw && p.raw.likeCount);
    el.innerHTML =
  ...
  function actionRow(type, id, title) {
    return '<div class="detail-actions">' +
      '<button class="act-btn act-like" data-like="' + UI.esc(id) + '" data-lt="' + type + '" type="button" aria-pressed="false">' +
  ...
  function renderComments(type, id) {
    var write = document.getElementById('cmtWrite');
    ...
    FB.getComments(type, id).then(function (cmts) {
      document.getElementById('cmtCnt').textContent = cmts.length ? cmts.length + '개' : '';
  ...
  function renderBoardList() {
    ...
    document.getElementById('boardToolbar').style.display = '';
  ...
  function renderBoardDetail(id) {
    var b = S.boards.find(function (x) { return x.docId === id; });
    var el = document.getElementById('boardContent');
    document.getElementById('boardToolbar').style.display = 'none';
  ...
  function renderEventList() {
    var el = document.getElementById('eventContent');
    document.getElementById('eventToolbar').style.display = '';
  ...
  function renderEventDetail(id) {
    ...
    var el = document.getElementById('eventContent');
    document.getElementById('eventToolbar').style.display = 'none';
  ...
  function showPage(name) {
    Object.keys(PAGE_VIEWS).forEach(function (k) { document.getElementById(PAGE_VIEWS[k]).hidden = k !== name; });
  }
  function parseHash() {
  ...
  function bindToolbars() {
    ...
    document.getElementById('boardSort').addEventListener('change', function (e) { B.sort = e.target.value; renderBoardList(); });
    ...
    document.getElementById('eventSort').addEventListener('change', function (e) { E.sort = e.target.value; renderEventList(); });
  }

  /* ================= 부팅 ================= */
  function start() {
    bindToolbars();
    UI.skelRows(document.getElementById('comPatchList'), 4);
    UI.skelRows(document.getElementById('comBoardList'), 4);
    UI.skelRows(document.getElementById('comEventList'), 3);
    UI.skelRows(document.getElementById('patchContent'), 5);
    UI.skelRows(document.getElementById('boardContent'), 5);
    UI.skelCards(document.getElementById('eventContent'), 3);
    ...
    loadAll().then(function () {
      ...
      /* 페이지 전용 배너 */
      function byPage(key) {
        return S.banners.filter(function (b) { return String(b.page || b.type || b.location || '').toLowerCase().indexOf(key) > -1; });
      }
      UI.fillBanner(document.getElementById('commBannerMedia'), null, byPage('communit').length ? byPage('communit') : S.banners.slice(0, 1));
      UI.fillBanner(document.getElementById('patchBannerMedia'), null, byPage('patch').length ? byPage('patch') : S.banners.slice(0, 1));
      UI.fillBanner(document.getElementById('boardBannerMedia'), null, byPage('board').length ? byPage('board') : S.banners.slice(0, 1));
      UI.fillBanner(document.getElementById('eventBannerMedia'), null, byPage('event').length ? byPage('event') : S.banners.slice(0, 1));
      route();
    }).catch(function (e) {
